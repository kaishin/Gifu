...
extension GIFAnimatable {
    ...
    /// - parameter loopCount: Desired number of loops, <= 0 for infinite loop.
    public func animate(withURL imageURL: URL, loopCount: Int = 0) {
        let session = URLSession.shared
        let task = session.dataTask(with: imageURL) { (data, response, error) in
            if let err = error {
                print("Error downloading gif:", err.localizedDescription, "at url:", imageURL.absoluteString)
            } else {
    ...
    /// - parameter imageURL: GIF image url.
    /// - parameter loopCount: Desired number of loops, <= 0 for infinite loop.
    public func prepareForAnimation(withURL imageURL: URL, loopCount: Int = 0, completionHandler: (() -> Void)? = nil) {
        let session = URLSession.shared
        let task = session.dataTask(with: imageURL) { (data, response, error) in
            if let err = error {
                print("Error downloading gif:", err.localizedDescription, "at url:", imageURL.absoluteString)
            } else {
                self.prepareForAnimation(withGIFData: data!, loopCount: loopCount, completionHandler: completionHandler)
            }
        }
        task.resume()
    }
